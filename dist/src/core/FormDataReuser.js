...
(function(global) {
    ...
    class FormDataReuser {
            ...
            return this.benchmark('getFieldMappings', async () => {
                try {
                    ...
                    const staticMappings = {
                        ...
                        // Campos do sistema OTRS
                        'ServiceID': { label: '🎯 Tipo de Atendimento', category: 'servico', priority: 'high' },
                        'Dest': { label: '📋 Fila de Atendimento', category: 'servico', priority: 'high' },
                        ...
                        'DynamicField_PRILocalidade': { label: '📍 Localidade', category: 'localizacao', priority: 'high' },
                        'DynamicField_localidade': { label: '📍 Localidade', category: 'localizacao', priority: 'high' },
        ...
        categorizeField(fieldId, fieldTitle) {
            try {
                ...
                const categoryRules = [
                    {
                        category: 'servico',
                        keywords: ['service', 'servico', 'serviço', 'tipo', 'fila', 'queue', 'dest', 'priority', 'prioridade', 'atendimento']
                    },
                    {
                        category: 'cliente',
                    ...
                    },
                    {
                        ...
                        keywords: ['sala', 'local', 'andar', 'predio', 'endereco', 'prédio', 'endereço', 'localidade']
                    },
                    {
                        category: 'patrimonio',
                    ...
                    for (const fieldId of Object.keys(mappings)) {
                        ...
                        try {
                            ...
                            if (field) {
                                ...
                                // Verificar também campos complexos do OTRS/Znuny
                                if (!value || value === '') {
                                    value = await this.captureComplexFieldValue(fieldId);
                                }
                            }
        ...
        /**
         * Capturar valor de campos complexos do OTRS com priorização especial
         * @param {string} fieldId - ID do campo
         * @returns {string|null} - Valor capturado
         */
        async captureComplexFieldValue(fieldId) {
            try {
                ...
                if (this.isPriorityField(fieldId)) {
                    ...
                    if (priorityValue) {
                        this.log('info', `Valor prioritário capturado para ${fieldId}: ${priorityValue}`);
                        return priorityValue;
                    }
                }

                // 1. Tentar campo de pesquisa padrão (_Search)
                ...
                if (searchField) {
                    ...
                    if (displayValue && displayValue !== '' && !displayValue.includes('Selecione')) {
                        this.log('info', `Valor encontrado em campo de pesquisa: ${fieldId}_Search`);
                        return displayValue;
                    }
                }

                // 2. Tentar campo select oculto original
                const hiddenSelect = this.getCachedElement(`#${fieldId}`);
                if (hiddenSelect && hiddenSelect.tagName === 'SELECT') {
                    const selectedOption = hiddenSelect.options[hiddenSelect.selectedIndex];
                    if (selectedOption && selectedOption.value && selectedOption.value !== '' && selectedOption.value !== '0') {
                        const value = selectedOption.textContent || selectedOption.innerText || selectedOption.value;
                        if (value && value.trim() !== '') {
                            this.log('info', `Valor encontrado em select oculto: ${fieldId}`);
                            return value.trim();
                        }
                    }
                }

                // 3. Tentar campo de exibição visual adjacente
                if (searchField) {
                    // Buscar por elementos com valor visual próximos
                    const parent = searchField.closest('.Field');
                    if (parent) {
                        const visualElements = parent.querySelectorAll('.Selected, .Value, [data-value], [title]');
                        for (const element of visualElements) {
                            const visualValue = element.textContent?.trim() || element.getAttribute('title')?.trim() || element.getAttribute('data-value')?.trim();
                            if (visualValue && visualValue !== '' && !visualValue.includes('Selecione')) {
                                this.log('info', `Valor encontrado em elemento visual: ${fieldId}`);
                                return visualValue;
                            }
                        }
                    }
                ...
                // 4. Tentar buscar por padrões específicos do OTRS
                const otrsPatterns = [
                    `#${fieldId}_Container .Selected`,
                    `#${fieldId}_Container .Value`,
                    `[data-field="${fieldId}"] .Selected`,
                    `[data-field="${fieldId}"] .Value`,
                    `.Field[data-field-id="${fieldId}"] .Selected`,
                    `.Field[data-field-id="${fieldId}"] .Value`
                ];

                for (const pattern of otrsPatterns) {
                    const element = this.getCachedElement(pattern);
                    if (element) {
                        const value = element.textContent?.trim();
                        if (value && value !== '' && !value.includes('Selecione')) {
                            this.log('info', `Valor encontrado com padrão OTRS: ${pattern}`);
                            return value;
                        }
                    }
                }

                // 5. Buscar em campos de erro/validação (podem conter valor atual)
                const errorField = this.getCachedElement(`#${fieldId}Error`);
                if (errorField && errorField.style.display !== 'none') {
                    const errorMsg = errorField.textContent?.trim();
                    // Se tem erro, pode ter valor atual na mensagem
                    const valueMatch = errorMsg?.match(/valor atual[:\s]*['"]([^'"]+)['"]/i);
                    if (valueMatch && valueMatch[1]) {
                        this.log('info', `Valor encontrado em campo de erro: ${fieldId}Error`);
                        return valueMatch[1];
                    }
                }

                this.log('debug', `Nenhum valor encontrado para campo complexo: ${fieldId}`);
                return null;
            } catch (error) {
                this.log('error', `Erro ao capturar campo complexo ${fieldId}`, error);
                return null;
            }
        }

        /**
         * Verificar se é um campo de prioridade alta
         * @param {string} fieldId - ID do campo
         * @returns {boolean}
         */
        ...
        /**
         * Capturar valor de campos prioritários com estratégias específicas
         * @param {string} fieldId - ID do campo
         * @returns {string|null} - Valor capturado
         */
        async capturePriorityFieldValue(fieldId) {
            try {
                switch (fieldId) {
                    case 'ServiceID':
                        return await this.captureServiceValue();
                    
                    case 'DynamicField_PRILocalidade':
                    case 'DynamicField_localidade':
                        return await this.captureLocalidadeValue(fieldId);
                    
                    case 'Dest':
                        return await this.captureDestValue();
                    
                    case 'PriorityID':
                        return await this.capturePriorityValue();
                    
                    default:
                        return null;
                }
            } catch (error) {
                this.log('error', `Erro ao capturar campo prioritário ${fieldId}`, error);
                return null;
            }
        }

        /**
         * Capturar valor do campo "Tipo de Atendimento" (ServiceID)
         * @returns {string|null}
         */
        async captureServiceValue() {
            try {
                // Estratégias específicas para ServiceID
                const strategies = [
                    () => {
                        const select = this.getCachedElement('#ServiceID');
                    ...
                    () => {
                        const serviceDisplay = this.getCachedElement('#ServiceID_Search');
                        return serviceDisplay?.value?.trim() || null;
                    },
                    () => {
                        const serviceText = this.getCachedElement('.ServiceSelection .Selected');
                        return serviceText?.textContent?.trim() || null;
                    },
                    () => {
                        const serviceField = this.getCachedElement('[name="ServiceID"] option:checked');
                        return serviceField?.textContent?.trim() || null;
                    }
                ];

                for (const strategy of strategies) {
                    const value = strategy();
                    if (value && value !== 'Selecionar Serviço' && value !== '-') {
                        this.log('info', `ServiceID capturado: ${value}`);
                        return value;
                    }
                }
                ...
                this.log('error', 'Erro ao capturar ServiceID', error);
                return null;
            }
        }

        /**
         * Capturar valor do campo "Localidade" (DynamicField_PRILocalidade ou DynamicField_localidade)
         * @param {string} fieldId - ID específico do campo
         * @returns {string|null}
         */
        async captureLocalidadeValue(fieldId = 'DynamicField_PRILocalidade') {
            try {
                // Estratégias específicas para Localidade baseadas no fieldId fornecido
                const strategies = [
                    () => {
                    ...
                    () => {
                        // Estratégia 3: Elemento visual adjacente (.InputField_Selection .Text)
                        const searchField = this.getCachedElement(`#${fieldId}_Search`);
                        if (searchField) {
                            const container = searchField.closest('.InputField_Container');
                    ...
                    () => {
                        ...
                        const displayValue = this.getCachedElement(`#${fieldId}_Search`)?.nextElementSibling?.textContent?.trim();
                        return displayValue || null;
                    },
                    () => {
                        // Estratégia 5: Container com data-field
                        const localidadeField = this.getCachedElement(`[data-field="${fieldId}"] .Selected`);
                        return localidadeField?.textContent?.trim() || null;
                    }
                ];

                for (const strategy of strategies) {
                    const value = strategy();
                    if (value && value !== 'Selecione uma localidade' && value !== '-' && value !== '') {
                        this.log('info', `Localidade capturada (${fieldId}): ${value}`);
                        return value;
                    }
                }
                ...
                this.log('error', `Erro ao capturar Localidade (${fieldId})`, error);
                return null;
            }
        }

        /**
         * Capturar valor do campo "Fila de Atendimento" (Dest)
         * @returns {string|null}
         */
        async captureDestValue() {
            try {
                const select = this.getCachedElement('#Dest');
                if (select && select.selectedIndex > 0) {
                    const selectedText = select.options[select.selectedIndex].textContent.trim();
                    // Extrair apenas o nome da fila (remover números e caracteres especiais)
                    const queueName = selectedText.replace(/^\d+\|\|/, '').trim();
                    this.log('info', `Dest capturado: ${queueName}`);
                    return queueName;
                }
                return null;
            } catch (error) {
                this.log('error', 'Erro ao capturar Dest', error);
                return null;
            }
        }

        /**
         * Capturar valor do campo "Prioridade"
         * @returns {string|null}
         */
        async capturePriorityValue() {
            try {
                const select = this.getCachedElement('#PriorityID');
                if (select && select.selectedIndex > 0) {
                    const priorityText = select.options[select.selectedIndex].textContent.trim();
                    this.log('info', `Prioridade capturada: ${priorityText}`);
                    return priorityText;
                }
                return null;
            } catch (error) {
                this.log('error', 'Erro ao capturar Prioridade', error);
                return null;
            }
        }
            ...
            return this.benchmark('createReusePopup', async () => {
                try {
                    ...
                    popup.innerHTML = `
                        ...
                        <div class="reuse-popup-help">
                            <small>🖱️ Clique nos dados para inserir no editor de texto</small>
                        </div>
                        <div class="reuse-popup-content">
                            <div class="reuse-popup-description">
                                <p>Clique nos dados para inserir no corpo da solicitação:</p>
                            </div>
        ...
        addReusePopupStyles() {
            ...
            style.textContent = `
                ...
                #helpOtrsFormReusePopup .reuse-popup-help {
        ...
        showInsertionFeedback(element, label) {
            try {
                ...
                // feedback.className = 'reuse-insertion-feedback'; não usar....
                // feedback.textContent = `✅ ${label}`; não usar....
