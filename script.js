...
function isTicketInService() {
  ...
  Array.from(document.querySelector('#Core_UI_AutogeneratedID_1').querySelectorAll('label')).forEach(label => {
...
function addDivAlert() {
  ...
  a.href = document.querySelector('.Actions').querySelectorAll('li')[12].querySelector('a').href
  a.textContent =
...
async function isCorrectQueue() {
  ...
  Array.from(children).forEach((child, index) => {
    if (child.textContent == "Fila:") {
      ...
      currentLevel = currentLevel.title.split('::')[1].split(' -')[0].replace('l', 'l ')
      return
    }
  });
